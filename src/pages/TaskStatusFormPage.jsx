      ...
      <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="text" className="w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="e.g. In Review" required />
          </div>

          <div>
            <label className="block text-sm font-bold text-gray-700 mb-2">System Status (Visibility)</label>
            <select className="w-full p-3 border rounded-lg bg-white outline-none cursor-pointer" value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value})}>
